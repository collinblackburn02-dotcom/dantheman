...
# ================= Attribute Summary Tables (from ORIGINAL CSV, no interaction) =================
# These tables read from `df` (the cleaned, unfiltered DataFrame),
# and only include SINGLE-ATTRIBUTE groups for each attribute.
...
def _singleton_summary_table(df_all: pd.DataFrame, attr_label: str, attr_col: str) -> pd.DataFrame:
    """
    Build a table for one attribute from the ORIGINAL df_all:
      - keep rows where this attribute is present (non-blank)
      - and ALL OTHER attribute columns are blank/NA
      - aggregate Visitors/Purchasers by attribute value
      - compute Conversion = 100 * Purchasers / Visitors
    """
    # Which attribute columns exist in original df?
    active_attr_cols = [c for c in attr_cols if c in df_all.columns]
    other_cols = [c for c in active_attr_cols if c != attr_col]

    # Select only rows where ONLY this attribute is set
...
st.markdown("---")
st.markdown("## 📑 Single-Attribute Summary Tables (unfiltered)")

# Fixed display order, regardless of UI toggles/filters
summary_order = [
    "Gender", "Age", "Homeowner", "Married", "Children",
    "Credit rating", "Income", "Net worth", "State",
    "Ethnicity (skiptrace)", "Department", "Seniority level", "Skiptrace credit",
...
for label in summary_order:
    # Only include if the attribute exists in the CSV
    ...
    tbl = _singleton_summary_table(df, label, col)
    st.markdown(f"**{label}**")
    if tbl.empty:
