...
if uploaded:
    ...
    seg_map = {
        ...
        'State': resolve_col(df, 'STATE'),  # Added for requested State column
    }
...
HAVING COUNT(*) >= ?
    ...
    def format_header(col):
        ...
        return formatted
    disp.columns = [format_header(col) for colopause

System: * The user's input was cut off at the end. However, based on the context and requirements provided, I can complete the response by implementing the requested changes to the table column order and names, ensuring the logic and rankings remain unaffected. The artifact will contain the full updated code with the table section modified to include the "Rank" column, reordered columns, and updated column names as specified.

### Completing the Response

The user’s input was cut off at "col" in the `format_header` function, but the requirements are clear:
- **Add a "Rank" column** at the far left, numbered 1, 2, 3, ... based on the sorted order.
- **Reorder columns** to: Rank, Visitors, Purchases, Conversion, Gender, Age, Homeowner, Married, Children, Credit rating, Income, Net worth, State, followed by SKU columns.
- **Update column names** to match the requested names, mapping them to existing columns.
- **Preserve SKU columns** at the end in their original order, with `SKU:` removed.
- **Maintain all existing logic and rankings**, including the header formatting (remove `skiptrace`, capitalize first letters).

I’ll complete the code by:
1. Adding the `Rank` column after sorting `res`.
2. Mapping the requested column names to existing columns using `seg_map` and handling `conv_rate` as `Conversion`.
3. Reordering columns in the specified order, including only those present in `disp`.
4. Applying the `format_header` function to clean up names (except for `conv_rate`) and renaming specific columns to match the requested names.
5. Ensuring SKU columns are appended last with cleaned names.

### Updated Table Section
The table section (replacing lines after `res = res.sort_values(sort_key, ascending=False).head(top_n)`) will:
- Add `Rank` as a new column.
- Define a mapping of requested names to current columns.
- Reorder columns to match the requested order, skipping any missing columns.
- Apply header formatting and rename specific columns.

Here’s the complete updated code:

<xaiArtifact artifact_id="60efaf0a-c86d-47d5-b446-62bc32914b85" artifact_version_id="38509d0c-27ef-4452-8bc5-3d1efad0afe4" title="app.py" contentType="text/python">
import streamlit as st
import pandas as pd
import numpy as np
import duckdb
