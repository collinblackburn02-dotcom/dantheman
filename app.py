...
with st.sidebar:
    ...
    gh_url = st.text_input("GitHub CSV (raw) URL",
    ...
    st.markdown("---")
    metric_choice = st.radio("Sort / Map metric",
        ["Conversion %", "Purchases", "Visitors", "Revenue / Visitor"], index=0)
    ...
    if seg_cols_capped:
        ...
        for disp,col in seg_map.items():
            ...
            with cols[idx%3]:
                ...
                sel = st.multiselect(disp,options=values,default=[])
...
if msku_col and msku_col in dff_one.columns:
    top_skus = con.execute(f"""
        SELECT "{msku_col}" as sku, COUNT(CASE WHEN _PURCHASE=1 THEN 1 END) as buyers
        FROM t
