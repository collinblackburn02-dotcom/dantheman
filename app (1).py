...
# Soft Green Colormap to fix the black-box issue
...
# ================ 3. Global Sidebar & Magic Branding =================
with st.sidebar:
    # URL Brand Scraper
    st.markdown("### 🎨 App Branding")
    url_input = st.text_input("Enter Website URL", placeholder="e.g. apple.com")
        ...
        with st.spinner("Extracting brand DNA..."):
            try:
                ...
                if color_meta:
                    ...
                    st.rerun() # Refresh to apply color instantly
            except Exception:
                st.warning("Could not extract branding automatically.")
...
# ================ 4. Two-Tab Layout =================
tab1, tab2 = st.tabs(["📊 Conversion Insights (Phase 1)", "🧬 Customer DNA (Phase 2 ICP)"])
...
with tab1:
    ...
    df_single = df_clean_single.groupby([selected_col]).agg(Visitors=('total_visitors', 'sum'), Purchases=('total_purchasers', 'sum'), Revenue=('total_revenue', 'sum')).reset_index()
    ...
    if not df_single.empty:
        ...
        df_single = df_single[df_single['Visitors'] >= min_visitors].sort_values(metric_map[metric_choice], ascending=is_ascending)
    ...
    for label, col_name in configs:
        ...
        grp = df_clean.groupby(col_name).agg(Visitors=('total_visitors', 'sum'), Purchases=('total_purchasers', 'sum')).reset_index()
...
with tab2:
    ...
    st.markdown('<p style="color: #666; margin-top: -5px; margin-bottom: 30px;">Upload orders to reveal the demographic DNA of paying customers.</p>', unsafe_allow_html=True)
    
    st.info("📦 **Upload your Order Data** (Required exactly: `Date`, `Email`, `Order ID`, `Total`)")
        ...
        try:
            ...
            if missing_cols:
                ...
                with st.spinner("Pushing orders to BigQuery and matching pixels..."):
                    ...
                    # 🛑 CRITICAL FIX: Strip dollar signs and commas BEFORE converting Total to numbers
                    ...
                    # Safely replace NaNs in demographic cols without breaking Boolean
                ...
                for index, (label, col_name) in enumerate(configs):
                    ...
                    if not grp.empty:
                        ...
                        # 🛑 CRITICAL FIX: Using soft custom_light_green so text is perfectly readable
